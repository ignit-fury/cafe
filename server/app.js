...
app.get('/api/seed', async (req, res) => {
  ...
  try {
    ...
    if (!adminExists) {
      ...
      await User.create({ name: 'Admin', email: 'admin@heartnbrew.in', password: hashedPassword, role: 'superadmin' });
    }

    for (const item of menuData) {
      await MenuItem.findOneAndUpdate({ name: item.name }, item, { upsert: true, new: true });
    }

    res.json({ message: 'Database seeded successfully! Admin: admin@heartnbrew.in / admin123' });
  } catch (error) {
